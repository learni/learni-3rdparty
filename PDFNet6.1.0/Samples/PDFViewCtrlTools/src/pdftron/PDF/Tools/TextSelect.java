...
class TextSelect extends Tool {
    ...
    public void onCreate() {
        ...
        mMenuTitles.add(new MenuEntry("copy",       getStringFromResId(R.string.tools_qm_copy)));
        ...
        mMenuTitles.add(new MenuEntry("crossout",   getStringFromResId(R.string.tools_qm_crossout)));
    ...
    public void selectText(float x1, float y1, float x2, float y2, boolean by_rect) {
        ...
        try {
            ...
            if (by_rect) {
                mPDFView.setTextSelectionMode(PDFViewCtrl.TEXT_SELECTION_MODE_RECTANGULAR);
            } else {
                mPDFView.setTextSelectionMode(PDFViewCtrl.TEXT_SELECTION_MODE_STRUCTURAL);
            }
            ...
            if (str.equals("copy")) {
                ...
                try {
                    ...
                    for (int pg = sel_pg_begin; pg <= sel_pg_end; ++pg)  {
                        ...
                        r.normalize();
                        double[] pts;
                        pts = mPDFView.convPagePtToScreenPt(r.getX1(), r.getY2(), pg);
                        ur.setX1(pts[0]);
                        ur.setY1(pts[1]);
                        pts = mPDFView.convPagePtToScreenPt(r.getX2(), r.getY1(), pg);
                        ur.setX2(pts[0]);
                        ur.setY2(pts[1]);
                        rect_list.add(ur);
                    }
