...
{
	REX(TRN_StamperCreate(&mp_impl, static_cast<TRN_SizeType>( size_type), a, b));
}
...
{
	REX(TRN_StamperSetTextAlignment(mp_impl, (TRN_TextAlignment)text_alignment));
}
...
{
	REX(TRN_StamperSetSize(mp_impl, (TRN_SizeType)size_type, a, b));
}
...
{
	REX(TRN_StamperShowsOnScreen(mp_impl, on_screen));
}

inline void Stamper::ShowsOnPrint(bool on_print)
{
	REX(TRN_StamperShowsOnPrint(mp_impl, on_print));
}
...
{
	REX(TRN_StamperSetAlignment(mp_impl, (TRN_HorizontalAlignment)horizontal_alignment, (TRN_VerticalAlignment)vertical_alignment));
}

inline void Stamper::SetPosition(double x, double y, bool percentage)
{
	REX(TRN_StamperSetPosition(mp_impl, x, y, percentage));
}
