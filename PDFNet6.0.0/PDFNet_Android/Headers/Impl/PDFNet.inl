...
inline PDFNet::CloudErrorCode PDFNet::ConnectToCloud(const char* username, const char* password)
{
	TRN_PDFNetCloudErrorCode result; 
	REX(TRN_PDFNetConnectToCloud(username, password,&result));
	return (CloudErrorCode)result;
}
...
inline void PDFNet::SetViewerCache(size_t max_cache_size, size_t max_zoom_factor)
{
	REX(TRN_PDFNetSetViewerCache(max_cache_size, max_zoom_factor));
}
...
inline bool PDFNet::SetTempPath(const UString& temp_path)
{
	RetBool(TRN_PDFNetSetTempPath(temp_path.mp_impl,&result));
}
