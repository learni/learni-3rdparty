    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        ...
        if (mAnnot != null) {
            ...
            if (mAnnot.equals(tempAnnot)) {
                ...
                setCtrlPts();
                mPDFView.invalidate((int) Math.floor(mBBox.left),
                        (int) Math.floor(mBBox.top),
                        (int) Math.ceil(mBBox.right),
                        (int) Math.ceil(mBBox.bottom));
        ...
        if (mAnnot != null
            ...
            try {
                ...
                if (mModifiedAnnot) {
                    ...
                    pts1_old = mPDFView.convPagePtToClientPt(r.getX1(), r.getY1(), mAnnotPageNum);
                    pts2_old = mPDFView.convPagePtToClientPt(r.getX2(), r.getY2(), mAnnotPageNum);
                    ...
                    mPDFView.update(mAnnot, mAnnotPageNum);
                } else if (prior_event_type == PDFViewCtrl.PRIOR_EVENT_PINCH
                        || prior_event_type == PDFViewCtrl.PRIOR_EVENT_DBLTAP) {
                    setCtrlPts();
                }
            } catch (Exception e1) {
