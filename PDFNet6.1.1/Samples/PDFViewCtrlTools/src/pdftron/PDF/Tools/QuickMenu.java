...
class QuickMenu extends PopupWindow {
    ...
    class MenuView extends View
    {
        ...
        public MenuView(Context context, ArrayList<Tool.MenuEntry> titles) {
            super(context);
            mContext = context;
            ...
            if (sz > 0) {
                ...
                for (int i = 0; i < sz; ++i) {
                    ...
                    btn.setLabel(titles.get(i).getText());
                    float text_width = mPaint.measureText(titles.get(i).getText());
                    float btn_width = text_width + mBtnHPadding * 2;
                    // Each button cannot be smaller than 6 times corner radius; so if there is only
                    // one button, arrow won't be placed at the corners.
        ...
        protected void onDraw(Canvas canvas) {
            ...
            for (int i = 0; i < sz; ++i) {
                ...
                if (btn.getPressed()) {
                    // Draw background of selected button; shrink the button a bit
    ...
    public QuickMenu(View anchor, int anchor_top, int anchor_left, int anchor_bottom, int anchor_right,
            List<Tool.MenuEntry> menu_titles, PopupWindow.OnDismissListener listener, boolean ha) {
        ...
        mMenuTitles = new ArrayList<Tool.MenuEntry>(menu_titles);
        mNoShadowLayer = false;

        if (android.os.Build.VERSION.SDK_INT < 14 && ha) {
            // It seems that the shadow layer doesn't work properly when hardware acceleration is
            // on before ICS.
    ...
    public String getSelectedTitle() {
        if (mMenuTitles != null) {
            ...
            return mMenuTitles.get(id).getText();
        } else {
            return null;
        }
    }
    ...
    public String getSelectedType() {
        if (mMenuTitles != null) {
            int id = mMenuView.getSelectedMenuId();
            return mMenuTitles.get(id).getType();
        } else {
            return null;
        }
    }
