...
class DialogColorPicker extends AlertDialog implements
    ...
    private void setUp(int color) {
        try {
            setTitle(mContext.getResources().getString(R.string.tools_dialog_colorpicker_title));

            int margin = 10;
            DisplayMetrics metrics = new DisplayMetrics();
            ((Activity) mContext).getWindowManager().getDefaultDisplay().getMetrics(metrics);
            margin = Math.round(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, margin, metrics));
