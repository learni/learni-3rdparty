                ...
                if (isInsideAnnot(x, y)) {
                    ...
                    if (mField.isValid() && !mField.getFlag(pdftron.PDF.Field.e_read_only)) {
                        ...
                        if (field_type == pdftron.PDF.Field.e_check) {
                            ...
                            double [] pts1, pts2;
                            pts1 = mPDFView.convPagePtToScreenPt(update_bbox.getX1(), update_bbox.getY1(), mAnnotPageNum);
                            pts2 = mPDFView.convPagePtToScreenPt(update_bbox.getX2(), update_bbox.getY2(), mAnnotPageNum);
        ...
        try {
            if (mField.isValid()) {
                ...
                if (just == Field.e_left_justified) {
                    mInlineEditBox.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
                } else if (just == Field.e_centered) {
                    mInlineEditBox.setGravity(Gravity.CENTER | Gravity.CENTER_VERTICAL);
                } else if (just == Field.e_right_justified) {
                    mInlineEditBox.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
                }
