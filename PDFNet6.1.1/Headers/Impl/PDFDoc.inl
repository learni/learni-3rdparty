...
inline PDFDoc::PDFDoc (const UChar* buf, size_t buf_size) {
	REX(TRN_PDFDocCreateFromBuffer((const char*)buf,buf_size,&mp_doc));
}
...
inline void PDFDoc::Save(const UString& path, UInt32 flags) {
	REX(TRN_PDFDocSave(mp_doc, path.mp_impl,flags,0));
}

#ifndef SWIG
inline void PDFDoc::Save(const UString& path, UInt32 flags, Common::ProgressMonitor* progress) {
	REX(TRN_PDFDocSave(mp_doc, path.mp_impl,flags, /*&progress->mp_impl*/NULL));
}
#endif
...
inline std::vector<unsigned char> PDFDoc::Save(UInt32 flags) {
	...
	REX(TRN_PDFDocSaveMemoryBuffer(mp_doc,flags,0,&size,(const char**)&buf));
	result.resize(size);
...
inline void PDFDoc::Save(const char* &out_buf, size_t& out_buf_size, UInt32 flags, Common::ProgressMonitor* progress) {
	REX(TRN_PDFDocSaveMemoryBuffer(mp_doc,flags,(TRN_ProgressMonitor*)progress,&out_buf_size,&out_buf));
}
#endif
...
inline void PDFDoc::InsertPages(int insertBeforeThisPage
								, PDFDoc& sourceDoc
								, int startPage
								, int endPage
...
#endif
	REX(TRN_PDFDocInsertPages(mp_doc, insertBeforeThisPage, sourceDoc.mp_doc, startPage, endPage, flag, m));
}
	...
	)
{
	...
	m = (TRN_ProgressMonitor*)monitor;
#endif
	REX(TRN_PDFDocInsertPageSet(mp_doc, insertBeforeThisPage, sourceDoc.mp_doc, sourcePageSet.mp_impl, flag, m));
}
	...
	)
{
	...
	m = (TRN_ProgressMonitor*)monitor;
#endif
	REX(TRN_PDFDocMovePages(mp_doc, moveBeforeThisPage, sourceDoc.mp_doc, startPage, endPage, flag, m));
}
...
{	
	...
	m = (TRN_ProgressMonitor*)monitor;
#endif
	REX(TRN_PDFDocMovePageSet(mp_doc, moveBeforeThisPage, sourceDoc.mp_doc, sourcePageSet.mp_impl, flag, m));
}
...
inline FDF::FDFDoc PDFDoc::FDFExtract(enum ExtractFlag flag) {
	TRN_FDFDoc result;
	REX(TRN_PDFDocFDFExtract(mp_doc, flag, &result));
...
#if defined(__iOS__)
inline PDFDoc* PDFDoc::__Create(ptrdiff_t impl) {
	return new PDFDoc((TRN_PDFDoc)impl);
}

inline ptrdiff_t PDFDoc::__GetHandle() {
...
//for xamarin use only
#if defined(__iOS__)
inline Context* Context::__Create(ptrdiff_t impl) {
	return new Context((TRN_OCGContext)impl);
}

inline ptrdiff_t Context::__GetHandle() {
