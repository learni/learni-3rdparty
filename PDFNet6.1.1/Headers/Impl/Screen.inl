...
//AA
inline SDF::Obj Screen::GetTriggerAction() const
{
	TRN_Obj result;
	REX(TRN_ScreenAnnotGetTriggerAction(mp_annot, &result));
	return SDF::Obj(result);
}

inline void Screen::SetTriggerAction(SDF::Obj action)
{
	REX(TRN_ScreenAnnotSetTriggerAction(mp_annot, action.mp_obj));
}
...
//R
inline int Screen::GetRotation() const
{
	int result;
	REX(TRN_ScreenAnnotGetRotation(mp_annot,&result));
	return result;
}

inline void Screen::SetRotation(int rot)
{
	REX(TRN_ScreenAnnotSetRotation(mp_annot, rot));
}
