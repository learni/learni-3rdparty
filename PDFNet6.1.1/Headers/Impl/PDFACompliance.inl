...
namespace pdftron { 
	...
	{
		REX(TRN_PDFAComplianceCreateFromFile(BToTB(convert), file_path.mp_impl, password, (enum TRN_PDFAConformance)conf, (enum TRN_PDFAErrorCode*)exceptions, num_exceptions, max_ref_objs, first_stop, &mp_pdfac));
	}
	...
	{
		REX(TRN_PDFAComplianceCreateFromBuffer(BToTB(convert), buf, (int)buf_size, password, (enum TRN_PDFAConformance)conf, (enum TRN_PDFAErrorCode*)exceptions, num_exceptions, max_ref_objs, first_stop, &mp_pdfac));
	}
	...
	inline std::vector<unsigned char> PDFACompliance::SaveAs(bool linearized){
		...
		REX(TRN_PDFAComplianceSaveAsFromBuffer(mp_pdfac, &size, (const char**)&buf, BToTB(linearized)));
		result.resize(size);
	...
	inline void PDFACompliance::SaveAs(const char* &out_buf, size_t& out_buf_size, bool linearized){
		REX(TRN_PDFAComplianceSaveAsFromBuffer(mp_pdfac, &out_buf_size,&out_buf, BToTB(linearized)));
	}
#endif
	...
	{
			int result;
			REX(TRN_PDFAComplianceGetRefObjCount(mp_pdfac, (enum TRN_PDFAErrorCode)id, &result));
			return (size_t)result;
	}

	inline size_t PDFACompliance::GetRefObj(ErrorCode id, size_t obj_idx){
			int result;
			REX(TRN_PDFAComplianceGetRefObj(mp_pdfac, (enum TRN_PDFAErrorCode)id, (int)obj_idx, &result));
			return (size_t)result;
	}
	...
	{
		const char* result;
		REX(TRN_PDFAComplianceGetPDFAErrorMessage((enum TRN_PDFAErrorCode)id, &result));
		return result;
	}
