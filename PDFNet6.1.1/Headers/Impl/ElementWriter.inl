...
inline void ElementWriter::Begin( Page& page, WriteMode placement, bool page_coord_sys, bool compress )
{
	REX(TRN_ElementWriterBeginOnPage(mp_writer,
		page.mp_page, static_cast<TRN_ElementWriterWriteMode>(placement), BToTB(page_coord_sys), BToTB(compress)));
}

inline void ElementWriter::Begin( SDF::Obj stream_obj_to_update, bool compress )
{
	REX( TRN_ElementWriterBeginOnObj(mp_writer, stream_obj_to_update.mp_obj, compress ) );
}
...
{
	REX(TRN_ElementWriterWriteBuffer(mp_writer,(const char*)&(data[0]),data.size()));
}
