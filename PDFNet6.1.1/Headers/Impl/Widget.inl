...
//AA
inline SDF::Obj Widget::GetTriggerAction() const
{
	TRN_Obj result;
	REX(TRN_WidgetAnnotGetTriggerAction(mp_annot, &result));
	return SDF::Obj(result);
}

inline void Widget::SetTriggerAction(SDF::Obj action)
{
	REX(TRN_WidgetAnnotSetTriggerAction(mp_annot, action.mp_obj));
}
...
//R
inline int Widget::GetRotation() const
{
	int result;
	REX(TRN_WidgetAnnotGetRotation(mp_annot,&result));
	return result;
}

inline void Widget::SetRotation(int rot)
{
	REX(TRN_WidgetAnnotSetRotation(mp_annot, rot));
}
