...
#ifdef _WIN32
inline Font Font::CreateTrueTypeFont2(SDF::SDFDoc& doc, const void* logfont, bool embed, bool subset)
{
	RetFont(TRN_FontCreateTrueTypeFont2(doc.mp_doc,logfont,BToTB(embed),BToTB(subset),&result));
}
#endif
inline Font Font::CreateCIDTrueTypeFont(SDF::SDFDoc& doc, const UString& font_path, bool embed, bool subset, Font::Encoding encoding)
{
	RetFont(TRN_FontCreateCIDTrueTypeFont(doc.mp_doc,font_path.mp_impl,BToTB(embed),BToTB(subset),(TRN_FontEncoding)encoding,&result));
}

#ifdef _WIN32
inline Font Font::CreateCIDTrueTypeFont2(SDF::SDFDoc& doc, const void* logfont, bool embed, bool subset, Font::Encoding encoding)
{
	RetFont(TRN_FontCreateCIDTrueTypeFont2(doc.mp_doc,logfont,BToTB(embed),BToTB(subset),(TRN_FontEncoding)encoding,&result));
}
					...
					Common::Matrix2D* transform)
{
	TRN_Bool result0;
	int oprs_sz,data_sz;
	REX(TRN_FontGetGlyphPath(mp_font,char_code,0,0,&oprs_sz,&data_sz,
		BToTB(conics2cubics),(TRN_Matrix2D*)transform, &result0));
	TRN_Bool result;
	...
	if (result0 && oprs_sz>0 && data_sz>0) {
		...
		TRN_FontGetGlyphPath(mp_font,char_code,&(out_oprs[0]),
			&(out_data[0]),&oprs_sz,&data_sz,BToTB(conics2cubics), 
	...
	return PathData(TBToB(result), out_oprs, out_data);
}

#ifndef SWIG
inline bool Font::GetGlyphPath(UInt32 char_code,
					...
					Common::Matrix2D* transform)
{
	...
	if (result0 && oprs_sz>0 && data_sz>0) {
		out_oprs.resize(oprs_sz);
		out_data.resize(data_sz);
		RetBool(TRN_FontGetGlyphPath(mp_font,char_code,&(out_oprs[0]),
			&(out_data[0]),&oprs_sz,&data_sz,BToTB(conics2cubics), 
			(TRN_Matrix2D*)transform, &result));
	}
	out_oprs.resize(0); out_data.resize(0);
	return TBToB(result0);
}
#endif
