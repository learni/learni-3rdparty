...
inline void Field::SetValue(SDF::Obj value)
{
	REX(TRN_FieldSetValue(&mp_field,value.mp_obj));
}

inline void Field::SetValue(const UString& value)
{
	REX(TRN_FieldSetValueAsString(&mp_field,value.mp_impl));
}

inline void Field::SetValue(const char* value)
{
	UString v(value, -1, UString::e_utf8);
	REX(TRN_FieldSetValueAsString(&mp_field,v.mp_impl));
}

inline void Field::SetValue(bool is_checked)
{
	REX(TRN_FieldSetValueAsBool(&mp_field,is_checked));
}
...
{
	TRN_Action result;
	REX(TRN_ActionCreateGotoWithKey(key,key_sz,dest.mp_dest,&result));
	return Action(result);
}
...
#if defined(__iOS__)
inline Action* Action::__Create(ptrdiff_t impl) {
	return new Action((TRN_Action)impl);
}

inline ptrdiff_t Action::__GetHandle() {
...
{
	TRN_Bool result;
	REX( TRN_Action_GetFormActionFlag(mp_action, static_cast<TRN_FormActionFlag>(flag), &result ) );
...
{
	REX( TRN_Action_SetFormActionFlag( mp_action, static_cast<TRN_FormActionFlag>(flag), static_cast<TRN_Bool>(value) ) );
}
...
inline void Page::FlattenField(Field filed)
{
	REX(TRN_PageFlattenField(mp_page, (TRN_Field*)&filed));
}
...
//for xamarin use only
#if defined(__iOS__)
inline Annot* Annot::__Create(ptrdiff_t impl) {
	return new Annot((TRN_Annot)impl);
}

inline ptrdiff_t Annot::__GetHandle() {
...
// BorderStyle
inline Annot::BorderStyle::BorderStyle(Style s, double b_width, double b_hr, double b_vr)
{
	REX(TRN_AnnotBorderStyleCreate((enum TRN_AnnotBorderStyleStyle)s, b_width,b_hr,b_vr,&mp_bs));
}

inline Annot::BorderStyle::BorderStyle(Style s, double b_width, double b_hr, double b_vr, const std::vector<double>& b_dash)
{
	size_t sz = b_dash.size();
	REX(TRN_AnnotBorderStyleCreateWithDashPattern((enum TRN_AnnotBorderStyleStyle)s, b_width,b_hr,b_vr, int(sz),(double*)(sz<1 ? 0 : &b_dash[0]),&mp_bs));
}

inline Annot::BorderStyle::BorderStyle(const BorderStyle& bs)
{
	REX(TRN_AnnotBorderStyleCopy(bs.mp_bs, &mp_bs));
}

inline Annot::BorderStyle::~BorderStyle()
{
	REX(TRN_AnnotBorderStyleDestroy(mp_bs));
	mp_bs=0;
}

inline void Annot::BorderStyle::Destroy()
{
	REX(TRN_AnnotBorderStyleDestroy(mp_bs));
	mp_bs=0;
}

inline Annot::BorderStyle& Annot::BorderStyle::operator=(const BorderStyle& bs) {
	if(this->mp_bs)
	{
		if( this == &bs ) 
			return *this;
...
inline bool operator==( const  Annot::BorderStyle& a, const  Annot::BorderStyle& b )
{
	TRN_Bool result;
...
inline Annot::BorderStyle Annot::GetBorderStyle() const
{
	TRN_AnnotBorderStyle result;
	REX(TRN_AnnotGetBorderStyle(mp_annot, &result));
	return Annot::BorderStyle(result);
}
...
inline Annot::BorderStyle::Style Annot::BorderStyle::GetStyle() const
{
	TRN_AnnotBorderStyleStyle result;
	REX(TRN_AnnotBorderStyleGetStyle( mp_bs, &result));
	return (Annot::BorderStyle::Style)result;	
}

inline void Annot::BorderStyle::SetStyle( Annot::BorderStyle::Style style)
{
	REX(TRN_AnnotBorderStyleSetStyle( mp_bs, (TRN_AnnotBorderStyleStyle)style));
}

inline double Annot::BorderStyle::GetHR() const
{
	double result;
	REX(TRN_AnnotBorderStyleGetHR(mp_bs,&result));
	return result;
}
inline void Annot::BorderStyle::SetHR(double hr)
{
	REX(TRN_AnnotBorderStyleSetHR(mp_bs, hr));
}

inline double Annot::BorderStyle::GetVR() const
{
	double result;
	REX(TRN_AnnotBorderStyleGetVR(mp_bs,&result));
	return result;
}
inline void Annot::BorderStyle::SetVR(double vr)
{
	REX(TRN_AnnotBorderStyleSetVR(mp_bs,vr));
}
inline double Annot::BorderStyle::GetWidth() const
{
	double result;
...
inline void Annot::BorderStyle::SetWidth(double width)
{
	REX(TRN_AnnotBorderStyleSetWidth(mp_bs,width));
}

inline std::vector<double> Annot::BorderStyle::GetDash() const
{
	double* result; int buf_length;
	REX(TRN_AnnotBorderStyleGetDashPattern(mp_bs,&buf_length,&result))
	std::vector<double> res(buf_length);
	if (buf_length>0) {
		memcpy(&(res[0]),result,buf_length*sizeof(double));
	}
	return res;
}

inline void Annot::BorderStyle::SetDash( const std::vector<double>& dash)
{
	size_t sz = dash.size();
	REX(TRN_AnnotBorderStyleSetDashPattern(mp_bs, int(sz), (sz<1 ? 0 : &dash[0]) ));
}


inline Annot::BorderStyle::BorderStyle(TRN_AnnotBorderStyle impl) { mp_bs=impl; }
...
inline void Annot::SetStructParent(const int keyval)
{
	REX(TRN_AnnotSetStructParent(mp_annot, keyval));
}
...
{
	REX(TRN_AnnotSetOptionalContent(mp_annot, (const TRN_Obj*)&oc));
}
