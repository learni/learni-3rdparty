...
inline void Flattener::SetPathHinting(bool path_hinting)
{
	REX(TRN_FlattenerSetPathHinting(mp_impl,BToTB(path_hinting)));
}

inline void Flattener::Process(PDF::Page& page, enum FlattenMode mode)
{
	REX(TRN_FlattenerProcessPage(mp_impl,page.mp_page,static_cast<TRN_FlattenMode>(mode)));
}
