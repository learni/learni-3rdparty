...
inline void Obj::SetString(const UChar* value, size_t size) {
	REX(TRN_ObjSetString(mp_obj,value,size));
}
...
#if defined(__iOS__)
inline SDFDoc* SDFDoc::__Create(ptrdiff_t impl) {
	return new SDFDoc((TRN_SDFDoc)impl);
}

inline ptrdiff_t SDFDoc::__GetHandle() {
...
#ifndef SWIG
inline bool SDFDoc::InitSecurityHandler(void* custom_data) 
{
	TRN_Bool result;
	REX(TRN_SDFDocInitSecurityHandler(mp_doc,custom_data,&result));
	return TBToB(result);
}
#endif
...
{
	TRN_Bool result;
	REX(TRN_SDFDocInitSecurityHandler(mp_doc,0,&result));
	return TBToB(result);
}
...
inline void SecurityHandler::InitPassword(const char* password) {
...
inline void SecurityHandler::ChangeUserPassword(const char* password ) {
...
inline void SecurityHandler::ChangeMasterPassword(const char* password ) {
