...
/**
 * FieldIterator is an iterator type that can be used to traverse a list 
...
public:
	 ...
	 * @param progress - A pointer to the progress interface. NULL if progress tracking is not required.
	 */
	void InsertPages(int insertBeforeThisPage
		, PDFDoc& sourceDoc
		, int startPage
		, int endPage 
	 ...
	 */
	void MovePages(int moveBeforeThisPage
		, PDFDoc& sourceDoc
		, int startPage
		, int endPage
		, enum InsertFlag flag
...
#if defined(__iOS__)
	static PDFDoc* __Create(ptrdiff_t impl);
	ptrdiff_t __GetHandle();
#endif //__iOS__
