...
public:
	...
	virtual void CreateTileProc(char* buffer, int originX, int originY, int width, int height, long long cellNumber, bool finalRender, bool predictionRender, int tiles_remaining,
	...
	static void StaticCreateTileProc(void* callingObject, char* buffer, int originX, int originY, int width, int height, int canvasNumber, long long cellNumber, bool finalRender, bool predictionRender, int tiles_remaining,
		bool first_tile, int canvas_width, int canvas_height, int cell_side_length, int cell_per_row, int cell_per_col, int thumb_nail_id )
	{
	...
	virtual void RemoveTileProc(int canvasNumber, long long cellNumber, int thumb_nail_id) {}
	
	static void StaticRemoveTileProc(void* callingObject, int canvasNumber, long long cellNumber, int thumb_nail_id) {
		Callback* f = (Callback*)callingObject;
		f->RemoveTileProc(canvasNumber, cellNumber, thumb_nail_id);
	}


	virtual void ThumbAsyncHandler(int page_num, bool was_thumb_found, const char* thumb_buf, int thumb_width, int thumb_height, void* custom_data) {}
	
	static void StaticThumbAsyncHandler(void* callingObject, int page_num, bool was_thumb_found, const char* thumb_buf, int thumb_width, int thumb_height, void* custom_data) {
		Callback* f = (Callback*)callingObject;
		f->ThumbAsyncHandler(page_num, was_thumb_found, thumb_buf, thumb_width, thumb_height, custom_data);
	}

	virtual void FindTextAsyncHandler(bool success, Selection& select) {}

	static void StaticFindTextProc(unsigned char success, TRN_PDFViewSelection select, void * user_data){
		Callback* f = (Callback*)user_data;
		Selection wrapper_sel(select);
		f->FindTextAsyncHandler((bool)success, wrapper_sel);
	}
