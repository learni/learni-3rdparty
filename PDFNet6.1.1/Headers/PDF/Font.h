...
public:
	 ...
	 *
	 * @param doc Document in which the external font should be embedded.
	 * @param logfont A pointer to a Windows LOGFONT structure that defines the 
	 ...
	 */
	#ifdef _WIN32
	 static Font CreateCIDTrueTypeFont2(SDF::SDFDoc& doc, const void* logfont, 
		bool embed = true, bool subset = true, 
		Encoding encoding=e_IdentityH);
	#endif
	 ...
	 * 
	 * @param char_code character to query
	 * @param out_oprs a vector of operators. The array is filled in by the method.
	 * @param out_data a vector of data points that represent arguments for operators. 
	 * The array is filled in by the method.
	 * @param conics2cubics if set to true converts all quadratic Bezier curves to cubic 
	 * Beziers, otherwise no conversion is performed.
	 * @param An optional matrix used to transform glyph data coordinates. If null/unspecified, 
	 * glyph data points will not be transformed.
	 * 
	 * @return false if the char_code was mapped to 'undefined character code'. 
	 ...
	 * Use GetType3GlyphStream instead.
	 */
	 bool GetGlyphPath(UInt32 char_code,
